...
import { frontLineToWorldX, shieldedFractionToFogOpacity, zecToUnitCount } from '../logic/mapping';
...
export default function Scene({ state }: { state: WarState }) {
  ...
  const shieldUnits = zecToUnitCount(state.supply.shieldedZec, { min: 24, max: 300, refZec: state.supply.totalSupply * 0.55 });
  const transparentUnits = zecToUnitCount(state.supply.transparentZec, { min: 24, max: 300, refZec: state.supply.totalSupply * 0.55 });
  ...
  return (
    <Canvas
      ...
      camera={{ fov: 45, near: 0.1, far: 200, position: [0, 22, 34] }}
    >
      <color attach="background" args={['#05070a']} />
      <fog attach="fog" args={['#05070a', 40, 95]} />
      ...
      />
      <pointLight position={[frontLineWorldX, 6, 0]} intensity={2} color={state.momentum >= 0 ? SHIELD_GLOW : TRANSPARENT_GLOW} distance={30} decay={2} />
      ...
      <FrontLine worldX={frontLineWorldX} color={state.momentum >= 0 ? SHIELD_COLOR : TRANSPARENT_COLOR} />
      ...
      <Army
        side="transparent"
        count={transparentUnits}
        frontLineWorldX={frontLineWorldX}
        color={TRANSPARENT_COLOR}
        emissiveColor={TRANSPARENT_GLOW}
        push={transparentPush}
      />
