...
/**
 * The battlefield ground between the two forts. Colors green/red based on
 * which side of the live front line (uFrontLine, a world-space X coord
 * derived from the real shielded fraction — see logic/mapping.ts) each
 * fragment falls on, with a pulsing glow band + scorched-earth cracks along
 * the line itself, and a tactical grid overlay for the "war map" feel.
 */
export const TerrainMaterial = shaderMaterial(
  ...
  /* glsl */ `
    ...
    void main() {
      ...
      // scorched-earth cracks radiating from the front line — a hint of past skirmishes
      float crack = vnoise(vec2(vWorldPos.z * 1.4, uTime * 0.02));
      float scorch = smoothstep(6.0, 0.0, distToFront) * step(0.62, crack) * 0.35;
      col = mix(col, vec3(0.02, 0.015, 0.012), scorch);

      float pulse = 0.55 + 0.45 * sin(uTime * 2.2 - distToFront * 0.6);
      float glow = exp(-distToFront * 0.3) * pulse;
